...
# CDK Change Safety
- **MANDATORY — always deploy via `scripts/deploy.sh <dev|prod>`**, never `cdk deploy` directly. The script enforces: SSO session check → `cdk diff` review → deploy → Liquibase migrations (post-deploy, uses new image) → ECS stabilization watch.
...
- **Liquibase runs after CDK deploy** so it uses the newly built Docker image containing the latest changesets. The deploy script handles this automatically. If Liquibase fails post-deploy, the API may fail health checks until migrations are applied manually.
